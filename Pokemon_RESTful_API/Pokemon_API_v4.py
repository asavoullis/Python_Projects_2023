...
def display_generation_pokemon(generation):
    ...
    if response.status_code == 200:
        data = response.json()
        pokemon_list = "\n".join([f"{pokemon['url'].split('/')[-2]}: {pokemon['name']}" for pokemon in data["pokemon_species"]])
        messagebox.showinfo(f"Pokemon in Generation {generation}", pokemon_list)
    else:
...
label_pokemon_name = tk.Label(window, text="Enter a Pokemon name:")
entry_pokemon_name = tk.Entry(window)

label_generation = tk.Label(window, text="Enter a generation number (1-9):")
entry_generation = tk.Entry(window)

label_pokedex_number = tk.Label(window, text="Enter a National Pokedex number:")
entry_pokedex_number = tk.Entry(window)

# Create buttons for options
button_get_data = tk.Button(window, text="Get data for a Pokemon", command=lambda: on_option_selected(1))
button_display_generation = tk.Button(window, text="Display Pokemon by Generation", command=lambda: on_option_selected(2))
button_search_by_pokedex = tk.Button(window, text="Search by National Pokedex Number", command=lambda: on_option_selected(3))
button_get_random = tk.Button(window, text="Get data for a Random Pokemon", command=lambda: on_option_selected(4))
button_quit = tk.Button(window, text="Quit", command=lambda: on_option_selected(5))
