...
def start_quiz_with_lives():
    global current_frame
    current_frame.pack_forget()
    play_mode_with_lives(word_dict, int(lives_entry.get()))

def start_quiz_without_lives():
    global current_frame
    current_frame.pack_forget()
    play_mode_without_lives(word_dict)

def start_quiz_with_timer():
    global current_frame
    current_frame.pack_forget()
    play_mode_with_timer(word_dict, float(timer_entry.get()))

def show_game_mode_selection():
    global current_frame
    current_frame = game_mode_frame
    current_frame.pack()
...
with_lives_button = tk.Button(root, text="Start Quiz with Lives", command=start_quiz_with_lives)
with_lives_button.pack()

without_lives_button = tk.Button(root, text="Start Quiz without Lives", command=start_quiz_without_lives)
without_lives_button.pack()

with_timer_button = tk.Button(root, text="Start Quiz with Timer", command=start_quiz_with_timer)
with_timer_button.pack()

exit_button = tk.Button(root, text="Exit", command=root.quit)
exit_button.pack()

# Create a frame for game mode selection
game_mode_frame = tk.Frame(root)
select_mode_label = tk.Label(game_mode_frame, text="Select a game mode:")
select_mode_label.pack()

with_lives_mode_button = tk.Button(game_mode_frame, text="With Lives", command=start_quiz_with_lives)
with_lives_mode_button.pack()

without_lives_mode_button = tk.Button(game_mode_frame, text="Without Lives", command=start_quiz_without_lives)
without_lives_mode_button.pack()

with_timer_mode_button = tk.Button(game_mode_frame, text="With Timer", command=start_quiz_with_timer)
with_timer_mode_button.pack()

current_frame = game_mode_frame  # Initial frame is the game mode selection
